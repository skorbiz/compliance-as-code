...
Use Python for model validation + normalized JSON output, then let Docusaurus consume generated JSON/MDX.

Pros:
- Keeps strong Python data pipeline.
- Moves presentation closer to Docusaurus over time.
- Reduces risk during migration.

Cons:
- Two-step workflow until fully consolidated.

# Recommendation

Start with option 2 (Python + Jinja) as the next iteration, then evaluate option 3 once the model schema is stable.

This gives immediate quality improvements with low risk, while keeping a path to a more Docusaurus-native architecture.
